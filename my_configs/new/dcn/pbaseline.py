...
img_norm_cfg = dict(
    mean=[155.038363, 155.038363, 155.038363], std=[97.321989, 97.321989, 97.321989], to_rgb=True)
# train_pipeline = [
#     dict(type='LoadImageFromFile'),
#     dict(type='LoadAnnotations', with_bbox=True),
#     dict(type='Resize', img_scale=[(1024,650), (1024,850)], multiscale_mode='range', keep_ratio=True),
...
train_pipeline = [
    ...
    # dict(type='Resize', img_scale=[(2048,1200), (2048,1800)], multiscale_mode='range', keep_ratio=True),
    dict(type='Resize', img_scale=(2048,1500), keep_ratio=True),
    dict(type='RandomFlip', flip_ratio=0.5),
    dict(type='Pad', size_divisor=32),
    dict(
...
data = dict(
    ...
    train=dict(
        ...
        dataset=dict(
            type=dataset_type,
        ann_file=data_root + 'annotations/ptrain2017.json',
        img_prefix=data_root + 'images/',
        pipeline=train_pipeline)),
    val=dict(
        type=dataset_type,
        ann_file=data_root + 'annotations/pval2017.json',
        img_prefix=data_root + 'images/',
        pipeline=test_pipeline),
    test=dict(
        type=dataset_type,
        ann_file=data_root + 'annotations/ptest2017.json',
...
load_from = "/datassd/yuejian/mmdetection/model/cascade_rcnn_dconv_c3-c5_r50_fpn_1x_coco_pretrained_weights_classes_4.pth"
resume_from = None
workflow = [
    ('train', 1)]
