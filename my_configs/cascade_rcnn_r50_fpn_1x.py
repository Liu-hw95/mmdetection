...
data_root = '/home/liusiyu/liuxin/mmdetection/data/' #数据集根目录
img_norm_cfg = dict(
...
data = dict(
	imgs_per_gpu=4,
	workers_per_gpu=6,
	##这里我改了点，直接填入原本的annotion_washed.json就行了
	train=dict(
		type=dataset_type,
		ann_file=data_root + 'train/tra_annotations.json', #修改成自己的训练集标注文件路径
		img_prefix=data_root + 'src/images/', #训练图片路径
        pipeline=train_pipeline),
	val=dict(
		type=dataset_type,
		ann_file=data_root + 'val/val_annotations.json', #修改成自己的验证集标注文件路径
		img_prefix=data_root + 'src/images/', #验证图片路径
        pipeline=test_pipeline),
	test=dict(
		type=dataset_type,
		ann_file=data_root + 'val/val_annotations.json',#修改成自己的验证集标注文件路径
		img_prefix=data_root + 'src/images/', #测试图片路径
        pipeline=test_pipeline))
# optimizer
optimizer = dict(
	type='SGD', lr=0.05, momentum=0.9, weight_decay=0.0001)#学习率的设置尤为关键：lr = 0.00125*batch_size
